...
When several rows match, invoke each of them. A slow Gradle build normally uses both `levyra-ci-workflows` and `levyra-context-efficiency`; a playback bug uses `levyra-real-engineering` plus `levyra-player`; a Compose jank issue uses `levyra-real-engineering` plus `levyra-compose`; a visual Compose redesign uses `levyra-design-taste` plus `levyra-compose`; a Desktop visual redesign uses `levyra-design-taste` plus `levyra-desktop`; a PR review uses `levyra-pr-review` plus all affected domain/security skills. If a skill turns out not to apply once read, say so in one line and continue rather than silently skipping it.
