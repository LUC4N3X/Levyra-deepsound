...
public class YoutubeStreamExtractor extends StreamExtractor {
    ...
    private static volatile boolean androidVrPlayerClientEnabled;

    public static void setAndroidVrPlayerClientEnabled(final boolean enabled) {
        androidVrPlayerClientEnabled = enabled;
    }
...
//        }
    ...
    public String getUploaderUrl() throws ParsingException {
        ...
        final String microformatChannelId = getPlayerMicroFormatRenderer()
                .getString("externalChannelId");
        if (!isNullOrEmpty(microformatChannelId)) {
            return YoutubeChannelLinkHandlerFactory.getInstance()
                    .getUrl("channel/" + microformatChannelId);
        }

        final String ownerChannelId = getVideoOwnerRendererOrEmpty()
                .getObject("navigationEndpoint")
                .getObject("browseEndpoint")
                .getString("browseId");
        if (!isNullOrEmpty(ownerChannelId)) {
            return YoutubeChannelLinkHandlerFactory.getInstance()
                    .getUrl("channel/" + ownerChannelId);
        }
    ...
    public String getUploaderName() throws ParsingException {
        ...
        final String microformatOwner = getPlayerMicroFormatRenderer()
                .getString("ownerChannelName");
        if (!isNullOrEmpty(microformatOwner)) {
            return microformatOwner;
        }

        final String ownerRendererName = getTextFromObjectOrNull(
                getVideoOwnerRendererOrEmpty().getObject("title"));
        if (!isNullOrEmpty(ownerRendererName)) {
            return ownerRendererName;
        }
    ...
    @Nonnull
    private JsonObject getVideoOwnerRendererOrEmpty() {
    ...
    @Nonnull
    private JsonObject getPlayerMicroFormatRenderer() {
        return playerMicroFormatRenderer == null ? new JsonObject() : playerMicroFormatRenderer;
    }

    @Nullable
    private static String getTextFromObjectOrNull(@Nonnull final JsonObject textObject) {
        try {
    ...
    public String getUploaderAvatarUrl() throws ParsingException {
        ...
        if (videoOwnerRenderer.has("thumbnail")) {
            final JsonArray thumbnails = videoOwnerRenderer
                    .getObject("thumbnail")
                    .getArray("thumbnails");
    ...
    public void onFetchPage(@Nonnull final Downloader downloader)
        ...
        if (androidVrPlayerClientEnabled) {
            try {
        ...
        if (!hasAnyPlayableData()) {
            throwIfErrors();
            throw new ContentNotSupportedException(
                    "YouTube returned no playable stream data from any enabled player client");
        }
