...
internal class YoutubeLocalStreamBridge(
    private val upstreamClient: OkHttpClient = ExtractorHttp.client.newBuilder()
        ...
        .build(),
    private val upstreamUrlValidator: (HttpUrl) -> Boolean = ::isAllowedYoutubeUrl,
    private val upstreamAddressResolver: (String) -> List<InetAddress> = ::resolveAddresses,
    private val upstreamAddressValidator: (InetAddress) -> Boolean = ::isPublicInternetAddress,
    ...
    private val securedUpstreamClient = upstreamClient.newBuilder()
        .dns(ValidatingDns(upstreamAddressResolver, upstreamAddressValidator))
        .build()
    ...
    private class ValidatingDns(
        ...
        private val validator: (InetAddress) -> Boolean
    ) : Dns {
        override fun lookup(hostname: String): List<InetAddress> {
            val addresses = resolver(hostname)
            if (addresses.isEmpty() || addresses.any { !validator(it) }) {
                throw UnknownHostException("Rejected bridge destination")
            }
            return addresses
        }
    }
    ...
    private companion object {
        ...
        fun isAllowedYoutubeUrl(url: HttpUrl): Boolean {
