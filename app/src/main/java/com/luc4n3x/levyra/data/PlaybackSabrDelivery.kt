...
internal data class SabrFormatCandidate(
    ...
    val audioTrackType: AudioTrackType? = null,
    val audioLocale: String? = null,
    val isAutoGeneratedAudio: Boolean = false,
...
internal fun orderSabrAudioCandidates(
    ...
    val byBitrate = if (preferHighestBitrate) {
        compareByDescending<SabrFormatCandidate> { it.averageBitrate.coerceAtLeast(it.bitrate) }
    } else {
        compareBy<SabrFormatCandidate> { it.averageBitrate.coerceAtLeast(it.bitrate) }
    }
    val comparator = compareByDescending<SabrFormatCandidate> { it.audioTier }
        .thenByDescending { it.tieBreakerBonus }
        .then(byBitrate)
    return usable.sortedWith(comparator)
}
