...
class LevyraHomeSnapshotCache(context: Context) {
    ...
    fun load(languageCode: String): LevyraHomeSnapshot? {
        ...
        return runCatching {
            ...
            val schema = rootJson.optInt("schema")
            if (schema !in MIN_SUPPORTED_SCHEMA..SCHEMA) return null
            ...
            val homeArtists = if (schema >= 10) parseArtists(rootJson.optJSONArray("homeArtists") ?: JSONArray()) else emptyList()
    ...
    private fun artistsToJson(artists: List<ArtistHit>): JSONArray {
        val array = JSONArray()
        artists
            .asSequence()
            ...
            .forEach { artist ->
                array.put(
                    JSONObject()
                        ...
                        .put("subscribers", artist.subscribers)
    ...
    private fun parseArtists(array: JSONArray): List<ArtistHit> {
        ...
        for (index in 0 until array.length()) {
            ...
            out.putIfAbsent(
                ...
                ArtistHit(
                    name = name,
                    subscribers = item.optString("subscribers").trim(),
