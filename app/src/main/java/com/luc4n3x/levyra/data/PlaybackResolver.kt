...
internal fun youtubeAudioTrackTier(
    format: JSONObject,
    resolvedUrl: String = "",
    preferredLanguage: String? = null
): Int = AudioLanguageIntelligence.parseFromFormat(format, resolvedUrl, preferredLanguage).tier
...
internal fun extractorAudioTrackTier(
    stream: AudioStream,
    preferredLanguage: String? = null
...
internal fun visionOsUserAgent(countryCode: String): String {
    ...
    return "com.google.visionos.youtube/1.04(RealityDevice17,1; U; CPU visionOS 26_6_0 like Mac OS X; $country)"
}
...
class PlaybackResolver private constructor(private val context: Context) {
    ...
    private val profiles = listOf(
        ClientProfile("VISIONOS", "1.04", "Apple Vision Pro", visionOsUserAgent("US"), false, 0L, 0, false),
    ...
    private suspend fun resolveAudioWithAndroidReel(
        ...
        val selectedAudio = firstPlayableAndroidReelAudio(
            candidates = androidReelAudioCandidates(
                adaptiveFormats = adaptiveFormats,
                sourceVideoId = sourceVideoId,
                cpn = cpn,
                audioQuality = audioQuality
            ),
            identity = reelStreamIdentity
        )
    ...
    private suspend fun resolveVideoWithAndroidReel(track: Track): Track {
        ...
        if (formats.length() == 0 && adaptiveFormats.length() == 0) {
            throw YoutubePlayerRequestException(null, "Android Reel non ha restituito formati video")
        }

        val muxedCandidates = androidReelVideoCandidates(
            formats = formats,
            sourceVideoId = sourceVideoId,
            cpn = cpn,
            muxed = true,
            fallbackLabel = "android-reel"
        )
        val videoOnlyCandidates = androidReelVideoCandidates(
            formats = adaptiveFormats,
            sourceVideoId = sourceVideoId,
            cpn = cpn,
            muxed = false,
            fallbackLabel = "adaptive"
        )
        val bestAudio = firstPlayableAndroidReelAudio(
            candidates = androidReelAudioCandidates(
                adaptiveFormats = adaptiveFormats,
                sourceVideoId = sourceVideoId,
                cpn = cpn,
        ...
        val bestAudioFormat = bestAudio?.first
        val bestAudioUrl = bestAudio?.second.orEmpty()
        ...
        val thumbnail = details
            ...
            .orEmpty()
            .ifBlank { track.largeThumbnailUrl.ifBlank { track.thumbnailUrl } }
        val selectedAudioUrl = if (selection.candidate.muxed) selection.candidate.url else bestAudioUrl
    ...
    private fun androidReelAudioCandidates(
        adaptiveFormats: JSONArray,
        sourceVideoId: String,
        cpn: String,
        audioQuality: String
    ): List<Triple<JSONObject, String, Int>> = buildList<Triple<JSONObject, String, Int>> {
        for (i in 0 until adaptiveFormats.length()) {
            ...
            val meta = AudioLanguageIntelligence.parseFromFormat(format, url, selectedPreferredAudioLanguage)
            add(
                Triple(
                    format,
                    url,
    ...
    private suspend fun firstPlayableAndroidReelAudio(
        candidates: List<Triple<JSONObject, String, Int>>,
        identity: YoutubeStreamClientIdentity
    ): Triple<JSONObject, String, Int>? {
        for (candidate in candidates) {
            if (verifyDirectAudioUrlFast(candidate.second, identity = identity)) return candidate
        }
        return null
    }

    private fun androidReelVideoCandidates(
        formats: JSONArray,
        sourceVideoId: String,
        cpn: String,
        muxed: Boolean,
        fallbackLabel: String
    ): List<LevyraVideoCandidate> = buildList<LevyraVideoCandidate> {
        for (i in 0 until formats.length()) {
            ...
            add(
                LevyraVideoCandidate(
                    ...
                    itag = format.optInt("itag", 0),
                    muxed = muxed,
                    label = format.optString("qualityLabel").ifBlank { fallbackLabel }
                )
            )
        }
    }
        ...
        youtubeHttpClient.newCall(request).execute().use responseUse@{ response ->
            ...
            val audioCandidates = buildList {
                for (i in 0 until adaptiveFormats.length()) {
                    ...
                    val meta = AudioLanguageIntelligence.parseFromFormat(format, preferredLanguage = selectedPreferredAudioLanguage)
                    add(
                        Triple(
                            format,
                            strictAudioSelectionScore(
    ...
    private fun sabrFormatCandidates(adaptiveFormats: JSONArray): List<SabrFormatCandidate> = buildList {
        for (index in 0 until adaptiveFormats.length()) {
            ...
            val isAudio = mime.startsWith("audio/", ignoreCase = true)
            val meta = if (isAudio) {
                AudioLanguageIntelligence.parseFromFormat(format, preferredLanguage = selectedPreferredAudioLanguage)
            } else null
            add(
                SabrFormatCandidate(
                    itag = format.optInt("itag", 0),
                    ...
                    mimeType = mime,
                    contentLength = meta?.contentLength?.takeIf { it > 0L } ?: (format.optString("contentLength").toLongOrNull() ?: 0L),
                    bitrate = format.optInt("bitrate", 0),
                    ...
                    xtags = meta?.xtags ?: format.optString("xtags"),
                    audioTrackType = when {
                        meta?.isExplicitOriginal == true -> AudioTrackType.ORIGINAL
                        meta?.isDescriptive == true -> AudioTrackType.DESCRIPTIVE
                        meta?.kind == AudioTrackKind.SECONDARY -> AudioTrackType.SECONDARY
                        meta?.kind == AudioTrackKind.HUMAN_DUB -> AudioTrackType.DUBBED
                        else -> null
                    },
                    audioLocale = meta?.language,
                    isAutoGeneratedAudio = meta?.isAutoDubbed ?: false,
                    isOriginalAudio = meta?.isExplicitOriginal ?: false,
                    audioTier = meta?.tier ?: 0,
                    tieBreakerBonus = meta?.tieBreakerBonus ?: 0
                )
            )
        }
    }
    ...
    private suspend fun resolveWithLevyraExtractor(
        ...
        val metadataTrack = track.withYoutubeSongMetadata(info.songMetadata)
        val artworkSafe = LevyraPersonalOrbit.preferAlbumArtwork(
            primary = metadataTrack,
            donor = metadataTrack.copy(thumbnailUrl = bestThumb, largeThumbnailUrl = bestThumb)
        )
        val durationMs = if (info.duration > 0L) info.duration * 1000L else metadataTrack.durationMs
