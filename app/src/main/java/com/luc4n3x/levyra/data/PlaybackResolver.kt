...
class PlaybackResolver private constructor(private val context: Context) {
            ...
            for (i in 0 until adaptiveFormats.length()) {
                ...
                add(
                    Triple(
                        format,
                        url,
                        strictAudioSelectionScore(youtubeAudioTrackTier(format), scoreAudioFormat(
                                mime = mime,
            ...
            val audioCandidates = buildList {
                for (i in 0 until adaptiveFormats.length()) {
                    ...
                    add(
                        Triple(
                            format,
                            strictAudioSelectionScore(youtubeAudioTrackTier(format), scoreAudioFormat(mime, itag, bitrate, formatAudioQuality, preferMp4Audio, audioQuality)),
    ...
    private fun scoreExtractorAudio(stream: AudioStream, preferMp4Audio: Boolean, audioQuality: String): Int {
        ...
        val trackTier = extractorAudioTrackTier(
