...
internal fun mergeOfflineMetadataTrack(original: Track, resolved: Track): Track {
    ...
    return resolved.copy(
        ...
        trackTotal = original.trackTotal.takeIf { it > 0 } ?: resolved.trackTotal,
        discTotal = original.discTotal.takeIf { it > 0 } ?: resolved.discTotal,
        composer = original.composer.ifBlank { resolved.composer },
        albumArtist = original.albumArtist.ifBlank { resolved.albumArtist },
        copyright = original.copyright.ifBlank { resolved.copyright },
        appleSongId = original.appleSongId.ifBlank { resolved.appleSongId },
        appleAlbumId = original.appleAlbumId.ifBlank { resolved.appleAlbumId },
...
internal fun Track.toRichM4aMetadata(artwork: ByteArray?, lyrics: String): LevyraM4aMetadata = LevyraM4aMetadata(
    ...
    trackId = appleSongId.ifBlank { id },
    albumId = appleAlbumId.ifBlank { albumBrowseId },
