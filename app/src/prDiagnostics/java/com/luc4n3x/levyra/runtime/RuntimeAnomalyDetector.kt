...
internal class RuntimeAnomalyDetector(
    ...
    private fun detectMemoryGrowth(event: MemorySampleEvent) {
        ...
        val monotonic = samples.zipWithNext().all { (previous, next) -> next.pssKb >= previous.pssKb }
        if (span >= thresholds.memoryMinimumSpanMs && growth >= thresholds.memoryGrowthKb && monotonic) {
    ...
    private fun detectCanvas(event: CanvasEvent) {
        if (
            event.action != CanvasAction.STARTED &&
            event.action != CanvasAction.RESTARTED &&
            event.action != CanvasAction.FALLBACK
        ) return
        countAndRaise(
            event.timestampMs,
            thresholds.shortWindowMs,
            ...
            it is CanvasEvent && (
                it.action == CanvasAction.STARTED ||
                    it.action == CanvasAction.RESTARTED ||
                    it.action == CanvasAction.FALLBACK
                )
        }
    }
