...
internal object RuntimeHooks {
    ...
    fun network(
        ...
        if (retry > 0) hot(RuntimeSignal.HOT_NETWORK_RETRY)
        RuntimeDiagnostics.record(
            NetworkEvent(
                timestampMs = nowMs(),
                uptimeMs = uptimeMs(),
    ...
    @Composable
    fun internalPanelOverlay() {
        InternalDiagnosticsOverlay()
    }
...
internal object RuntimeDiagnostics {
    ...
    fun start(context: Context) {
        ...
        val resourceMarker = context.resources.openRawResource(R.raw.levyra_pr_diagnostics_marker)
            .bufferedReader()
            .use { it.readText().trim() }
        check(resourceMarker == "LEVYRA_PR_DIAGNOSTICS_RESOURCE_V1")
...
private fun Int.networkCategory(): NetworkCategory = when (this) {
    ...
    RuntimeSignal.NETWORK_HTTP -> NetworkCategory.HTTP
