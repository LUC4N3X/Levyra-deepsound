...
'''
    new_player_screen = '''private data class PlayerCollapseActions(
    val collapse: () -> Unit,
    val dragStart: () -> Unit,
    val drag: (Float) -> Unit,
...
'''
    source = replace_once(source, old_player_screen, new_player_screen, "PlayerScreen signature")

    old_player_call = '''                    PlayerScreen(
                        viewModel = playerViewModel,
...
'''
    source = replace_once(source, old_player_call, new_player_call, "PlayerScreen call")
    source = replace_once(
        source,
        "                    onClick = onCollapse\n",
...
'''
    source = replace_once(source, old_mini_call, new_mini_call, "MiniPlayer call")

    old_gesture_call = '''                    PlayerGestureLayer(
...
'''
    new_gesture_call = '''                    PlayerGestureLayer(
...
'''
    source = replace_once(source, old_gesture_call, new_gesture_call, "PlayerGestureLayer call")

    gesture_pattern = re.compile(
        r"@Composable\nprivate fun PlayerGestureLayer\(.*?\n\}\n\n@Composable\nprivate fun PlayerYoutubeCommentsSheet",
        re.DOTALL,
    )
    new_gesture_layer = '''@Composable
private fun PlayerGestureLayer(
    config: PlayerGestureConfig,
    mediaActions: PlayerGestureMediaActions,
    uiActions: PlayerGestureUiActions,
...
@Composable
private fun PlayerYoutubeCommentsSheet'''
    source, gesture_count = gesture_pattern.subn(new_gesture_layer, source, count=1)
    if gesture_count != 1:
        raise RuntimeError(f"PlayerGestureLayer body: expected one match, found {gesture_count}")

    old_mini_signature = '''@Composable
...
private fun MiniPlayer(
    ...
    val strings = LocalLevyraStrings.current
'''
    source = replace_once(source, old_mini_signature, new_mini_signature, "MiniPlayer signature")

    mini_helper = '''private fun handleMiniPlayerDragEvent(
    event: PlayerDragEvent,
    playbackActions: MiniPlayerPlaybackActions,
    expansionActions: MiniPlayerExpansionActions,
...
private fun MiniPlayer(
'''
    source = replace_once(source, "@Composable\nprivate fun MiniPlayer(\n", mini_helper, "MiniPlayer helpers")

    mini_start = source.index("@Composable\nprivate fun MiniPlayer(\n")
    mini_end = source.index("\n@Composable\nprivate fun MiniPlayerToggleButton", mini_start)
    mini_block = source[mini_start:mini_end]
    old_mini_gesture = '''            ) { event ->
                when (event) {
...
'''
    mini_block = replace_once(mini_block, old_mini_gesture, new_mini_gesture, "MiniPlayer gesture")
    mini_block = mini_block.replace("onClick = onOpen", "onClick = playbackActions.open")
    mini_block = mini_block.replace("onToggle = onToggle", "onToggle = playbackActions.toggle")
    mini_block = mini_block.replace("onClick = onNext", "onClick = playbackActions.next")
    mini_block = mini_block.replace("onClick = onClose", "onClick = playbackActions.close")
    source = source[:mini_start] + mini_block + source[mini_end:]
    ...
    if leftovers:
        raise RuntimeError(f"Old player API fragments remain: {leftovers}")
