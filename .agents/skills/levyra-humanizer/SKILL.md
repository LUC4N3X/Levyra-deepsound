...
This repository-native skill vendors the Humanizer 2.11.2 writing method from `blader/humanizer`. Levyra changes only the skill name and front matter so it satisfies the repository's native skill contract. Apply the upstream rewrite method below as written. Levyra's release-note evidence, required workflow headings, version fields, Markdown targets, and truthfulness rules remain authoritative and must not be altered or invented during humanization.
