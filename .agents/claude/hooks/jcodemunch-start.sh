...
if [[ -n "$project_dir" && -n "$python_cmd" && -f "$runtime" ]]; then
  ...
  status="jCodeMunch: repository root unavailable. Continue with Claude native tools and report the tooling mismatch once. Token savings never override correctness."
else
